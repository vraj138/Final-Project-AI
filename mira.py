    ...
    for epo in range(0, epoch):
        ...
        for i in range(0, len(x_train)):
            ...
            if predicted_label != curr_label:
                error = error + 1
                weight_Diff = (
                    weights[:, predicted_label] - weights[:, curr_label])
                dotProduct_feature = np.dot(
                    individual_image_pixels.T, individual_image_pixels)

                tau_numerator = np.dot(
                    weight_Diff.T, individual_image_pixels) + 1
                tau_denom = 2 * dotProduct_feature
                tau = tau_numerator / tau_denom

                weights[:, curr_label] = weights[:, curr_label] + \
                    (tau * individual_image_pixels[:])
                weights[:, predicted_label] = weights[:,
                                                      predicted_label] - (tau * individual_image_pixels[:])
...
def mira(training_images_file, training_labels_file, test_images_file, test_labels_file, type):
    ...
    if (type == "Digits"):
        ...
        Y_train_labels = rd.load_label(training_labels_file, 451)
